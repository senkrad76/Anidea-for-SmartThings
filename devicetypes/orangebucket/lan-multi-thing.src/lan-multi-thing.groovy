 ...
 * Changes:
 *
 * 1.0.0		(10/10/2018)	Rename the app with a more generic name and reset the
 *								version number.
 *
...
metadata
{
	definition (name: "LAN Multi-Thing", namespace: "orangebucket", author: "Graham Johnson")
    {
		capability "Actuator"
                ...
                body.attribute.each
				{
                	...
                	if (myname == "eta")
                	{
                		try
                    	{
                    		...
                    		log.info "${device}: parse attribute $myname $myvalue (${mytime})"
                    	...
                    	catch(Exception e)
                    	{
                    		...
                    		log.info "${device}: parse attribute $myname (value invalid)"
					...
					{
                 		log.info "${device}: parse attribute ${myname} $myvalue.value $myvalue.unit"      
                		sendEvent(name: myname, value: myvalue.value, unit: myvalue.unit, isStateChange: true)
                    }
                    else
               		{
                		log.info "${device}: parse attribute ${myname} ${myvalue}"      
                ...
                body.state.each
				{
                	...
                	if (myname.contains("date"))
                	{
                		try
                    	{
                    		...
                    		log.info "${device}: parse state $myname $myvalue (${mytime})"
                    	...
                    	{
                    		// The returned value is not an ISO8601 date.
                    		log.info "${device}: parse state $myname (value invalid)"
               		...
               		{
                		log.info "${device}: parse state ${myname} ${myvalue}"      
