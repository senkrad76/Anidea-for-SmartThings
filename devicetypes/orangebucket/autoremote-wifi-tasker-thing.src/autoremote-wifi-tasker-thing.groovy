 ...
 * ============================
 * Connects to devices running the AutoRemote WiFi Service on the local LAN and issues
 * commands in the AutoApps command format to be processed by Tasker. The commands are
 * just URLs so the remote devices can do what they like in response.
 *
 * Listens for 'pings' from the remote device and accepts state change events in JSON
 * format for a number of capabilities.
 *
 ...
 *						2.2		(09/10/2018)	Further refinement. Add ability to
...
metadata
{
	definition (name: "AutoRemote WiFi Tasker Thing", namespace: "orangebucket", author: "Graham Johnson")
    {
		capability "Actuator"
