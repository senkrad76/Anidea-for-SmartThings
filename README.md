...
The handlers for the Mijia and Aqara Door and Window sensors only truly differed in the fingerprints of the devices and how exactly the same on/off event was handled, so one handler now covers both options. Please be aware that the handler sets the status of the sensor to 'closed' when it is installed (or updated via the IDE) as initialising the attributes just seems to make things work better. The manual commands to 'reset' the status to a known state have been retained, but renamed to 'open()' and 'close()' to match those in the Simulated Contact Sensor.
